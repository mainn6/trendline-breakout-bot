...
- 거래량 ≥ $10M USDT 페어 자동 선정 (~150개)
- 매일 00:05 UTC에 추세선 자동 재계산 (Swing High 기반)
- WebSocket 실시간 가격 추적 (Binance Futures)
- **3단계 알람**:
...
- `score = (2 + touches) × 100 + length − above × 50 + proximity_bonus`
- **touches**: 다른 swing high가 직선에 ±0.5% 이내 닿는 개수
- **proximity**: 현재가가 추세선에 가까울수록 가산
...
├── binance_ws.py          # WebSocket 멀티플렉스 (kline_1d + kline_1m)
